...
#else
    const char* build_str = "-DUSE_HIP=OFF  -DOPTIMIZE_FIELDS=ON -DOPTIMIZE_ARRAYS=ON -DBUILD_SAMPLES=OFF -DBUILD_STANDALONE=OFF -DBUILD_SHARED_LIBS=ON -DMPI_ENABLED=ON -DOPTIMIZE_MEM_ACCESSES=ON -DMAX_THREADS_PER_BLOCK=512 -DBUILD_TESTS=ON -DDSL_MODULE_FILE=solve_two.ac -DDSL_MODULE_DIR=../../test/magnetic-decay/DSL/local";
#endif
