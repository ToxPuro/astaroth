...
main(void)
{
    ...
    const auto cg_2nd_order_graph = acGetOptimizedDSLTaskGraph(cg_2nd_order_step);
    ...
    while(relative_residual > 1e-15 && step < max_step)
    {
    	...
    	acGridExecuteTaskGraph(cg_2nd_order_graph,1);
    ...
    const AcReal l2_diff_2nd_order = sqrt(info[AC_ds].x*info[AC_ds].y*info[AC_ds].z*acDeviceGetOutput(acGridGetDevice(),AC_2nd_order_l2_from_analytical_solution));
    fprintf(stderr,"L2 diff to solution: %.14e\n",l2_diff);
    fprintf(stderr,"L2 4th order diff to solution: %.14e\n",l2_diff_4th_order);
    fprintf(stderr,"L2 2nd order diff to solution: %.14e\n",l2_diff_2nd_order);
