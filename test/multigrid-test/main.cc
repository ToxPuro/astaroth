...
store_and_prolong(AcMesh mesh, int level)
{
	...
	acGridExecuteTaskGraph(acGetOptimizedDSLTaskGraph(gmg_prolong_solution),1);
    	--level;
    }
...
const int MAX_GMG_LEVEL = 4;
bool exit_after = false;
void
gmg_level_step(const int level)
{
  const auto info = acGridGetLocalMeshInfo();
  acDeviceSetInput(acGridGetDevice(),AC_GMG_LEVEL,(GMG_LEVEL)level);
  ...
  if(exit_after) exit(EXIT_SUCCESS);
...
main(void)
{
    ...
    acSetGridMeshDims(33,33,33, &info);
    acSetLocalMeshDims(33,33,33, &info);
	...
	acDeviceSetInput(acGridGetDevice(),AC_GMG_LEVEL,(GMG_LEVEL)i);
    	const auto sor_graph         = acGetOptimizedDSLTaskGraph(sor_red_black_step);
    	const auto gm_sor_graph         = acGetOptimizedDSLTaskGraph(gmg_sor_red_black_step);
