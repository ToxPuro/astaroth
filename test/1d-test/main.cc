...
main(void)
{
    ...
    acSetGridMeshDims(64, 1, 1, &info);
    //TP: backwards compatibility
    acSetLocalMeshDims(64, 1, 1, &info);
    ...
    const auto comm_graph = acGetOptimizedDSLTaskGraph(bcs);
    ...
    if(pid == 0)
    {
      AcReal* derxx = (AcReal*)malloc(sizeof(AcReal)*mm.x);
      ...
      for (int step_number = 0; step_number < NUM_INTEGRATION_STEPS; ++step_number) {
          ...
          for (size_t k = nz_min; k < nz_max; ++k) {
              for (size_t j = ny_min; j < ny_max; ++j) {
                  for (size_t i = nx_min; i < nx_max; ++i) {
          		const int index = IDX(i,j,k);
          		model.vertex_buffer[UU][index] = model.vertex_buffer[UU][index] + dt*(derxx[index]);
                  }
              }
          }
      }
      ...
      const AcResult res = acVerifyMesh("1d-heat", model, candidate);
      if (res != AC_SUCCESS) {
          retval = res;
          WARNCHK_ALWAYS(retval);
      }
