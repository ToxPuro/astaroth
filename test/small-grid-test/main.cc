...
main(void)
{
    ...
    acSetGridMeshDims(3, 3, 3, &info);
    acSetLocalMeshDims(3, 3, 3, &info);
    ...
    if(pid == 0)
    {
      for (int step_number = 0; step_number < NUM_INTEGRATION_STEPS; ++step_number) {
          for (size_t k = nz_min; k < nz_max; ++k) {
              for (size_t j = ny_min; j < ny_max; ++j) {
          		...
          		model.vertex_buffer[UU][index] += dt*model.vertex_buffer[UU][index];
                  }
              }
          }
      }
      ...
      const AcResult res = acVerifyMesh("small-grid", model, candidate);
      if (res != AC_SUCCESS) {
          retval = res;
          WARNCHK_ALWAYS(retval);
      }
    ...
    if (pid == 0)
        fprintf(stderr, "Small grid complete: %s\n",
