...
//TP: we used fixed_boundary to be compatible with PC that does not apply e.g. 
//fixed_boundary
Kernel sld_calc_char_speed(PC_SUB_STEP_NUMBER step_num)
{
	...
	if(AC_lslope_limit_diff__mod__cdata && step_num == AC_num_substeps__mod__cdata-1)
	{
		...
		if(vertexIdx == (int3){3,18,34})
		{
			print("GPU SLD AA: %.14e,%.14e,%14e\n",
					AAX[vertexIdx.x][vertexIdx.y][vertexIdx.z],
					AAY[vertexIdx.x][vertexIdx.y][vertexIdx.z],
					AAZ[vertexIdx.x][vertexIdx.y][vertexIdx.z]
			     )
			bb = curl(AA)
			print("GPU SLD BB: %.14e,%.14e,%14e\n"
					,bb.x
					,bb.y
					,bb.z
			     )

			mu = AC_mu0__mod__cdata
			lnrho = value(LNRHO)
			inv_rho = exp(-lnrho)
			inv_mu  = 1.0/mu
			alfven_speed = sqrt(dot(bb,bb)*inv_rho*inv_mu)
			print("Alfven speed GPU: %.14e\n"
					,alfven_speed
			     )
			print("Hydro speed GPU: %.14e\n"
					,norm(value(UU))
			     )
			print("Sound speed GPU: %.14e\n",energy_sld_sound_speed())
		}
