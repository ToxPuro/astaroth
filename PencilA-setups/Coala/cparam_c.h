...
;
  ...
  #include "cparam.local_c.h"
#endif
  const int  nx=nxgrid/nprocx,ny=nygrid/nprocy,nz=nzgrid/nprocz;
  const int  max_n = MAX(nx,MAX(ny,nz)), nxy=nx*ny, nxz=nx*nz, nyz=ny*nz;
  ...
  const int  mreduce=6;
  const int  ninit=5;
  const int  max_diagnostics=100;
  const int  fnlen=2048,intlen=21,bclen=3,labellen=40,linelen=fnlen+256,iomsglen=256;
  const int  datelen=30,max_col_width=64,nscbc_len=24,fmtlen=30;
  const int  mseed=256;
  ...
  const real  min_ts=1e-37;
  const real  impossible=3.9085e37;
  const int  impossible_int=21474836;
  const int  root=0;
  ...
  const int  ilabel_max_reciprocal=-5,ilabel_extr_lim=-9;
  ...
  const int  ilabel_sum_lim=9,ilabel_complex=100,ilabel_ignore=99;
  const real  lntwo=real(0.69314718055995E0);
  ...
  const double  g_newton_cgs=6.67428E-8;
  const double  density_scale_cgs=1.2435E21;
  const double  n_avogadro_cgs=6.022E23;
  const double  alpha_fine=7.2973525643E-3;
  const double  sigma_thomson_cgs=6.652458732160E-25;
  const double  e_cgs=4.8032047E-10;
  const double  chypercharge=41./12.;
  const double  mass_zboson=7.48e-18;
  const double  mass_zboson_gev=91.2;
  ...
  const int  n_pdfs_max=10, n_cspec_max=10;
  const int  mname=100;
  const int  mname_half=20;

  const int  nline=1200;

#if IN_DSL
  #include "../../../cparam_enum_c.h"
#else
  #include "cparam_enum_c.h"
#endif
