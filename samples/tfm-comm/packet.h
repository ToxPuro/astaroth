  ...
  public:
    Packet(const Shape& in_local_mm, const Shape& in_local_nn, const Index& in_local_rr,
        ...
        : cart_comm(MPI_COMM_NULL),
          ...
          segment(in_segment),
          send_buffer(std::make_unique<Buffer<T>>(n_aggregate_buffers * prod(in_segment.dims))),
          recv_buffer(std::make_unique<Buffer<T>>(n_aggregate_buffers * prod(in_segment.dims))),
    ...
    void launch(const MPI_Comm& parent_comm, const PackPtrArray<T*> inputs)
    {
        ...
        // Post recv
        const int tag = 0;
        ERRCHK_MPI_API(MPI_Irecv(recv_buffer->data, as<int>(count), get_dtype<T>(), recv_neighbor,
        ...
        ERRCHK_MPI_API(MPI_Isend(send_buffer->data, as<int>(count), get_dtype<T>(), send_neighbor,
