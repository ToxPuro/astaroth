...
template <typename T>
void
set_ndarray(const T value, const size_t ndims, const size_t* dims, const size_t* subdims,
            const size_t* start, T* arr)
{
    if (ndims == 0) {
        *arr = value;
    }
...
static void
pack_(const PackKernelParams params)
{
...
static void
unpack_(const PackKernelParams params)
{
    ERRCHK(params.pack_buffer != NULL);
    ERRCHK(params.buffers != NULL);
