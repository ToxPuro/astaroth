...
template <typename T> class BufferExchangeTask {
  private:
    Buffer<T> first_staging_buffer;
    Buffer<T> second_staging_buffer;
    cudaStream_t stream;
  ...
  public:
        ...
        : stream(nullptr)
    {
        if (type == BUFFER_EXCHANGE_DTOH) {
            first_staging_buffer  = Buffer<T>(max_count, BUFFER_DEVICE);
            second_staging_buffer = Buffer<T>(max_count, BUFFER_HOST_PINNED);
        }
        else if (type == BUFFER_EXCHANGE_HTOD) {
            first_staging_buffer  = Buffer<T>(max_count, BUFFER_HOST_PINNED_WRITE_COMBINED);
            second_staging_buffer = Buffer<T>(max_count, BUFFER_DEVICE);
        }
        else {
