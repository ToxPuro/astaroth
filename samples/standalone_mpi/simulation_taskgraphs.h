...
get_simulation_graph(int pid, Simulation sim)
{
    ...
    auto make_graph = [pid](Simulation sim) -> AcTaskGraph *{
        ...
        switch(sim){
          ...
          {
#if LSHOCK
	     // This still has to be behind a preprocessor feature, because e.g., VTXBUF_SHOCK is not defined in general
	  ...
	  default:
             acLogFromRootProc(pid, "ERROR: no custom task graph exists for selected simulation. This is probably a fatal error.\n");
	     return nullptr;
	}
    };
