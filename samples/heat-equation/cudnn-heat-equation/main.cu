...
int main(void) {
    ...
    cudnnSetConvolution2dDescriptor(convolution_desc, pad_h, pad_w, str_h, str_w, dil_h, dil_w, CUDNN_CONVOLUTION, dtype);
    ...
    int fh_out;
    int fw_out;
    cudnnGetConvolution2dForwardOutputDim(convolution_desc, input_desc, filter_desc, &fn_out, &fc_out, &fh_out, &fw_out);

    cudnnTensorDescriptor_t output_desc;
    cudnnCreateTensorDescriptor(&output_desc);
    cudnnSetTensor4dDescriptor(output_desc, format, dtype, fn_out, fc_out, fh_out, fw_out);
    float* output;
    cudaMallocManaged((void**)&output, fn_out * fc_out * fh_out * fw_out * sizeof(output[0]));

    // Algorithm
    const cudnnConvolutionFwdAlgo_t algorithm = CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM;
    // const cudnnConvolutionFwdAlgo_t algorithm = CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM;
    // const cudnnConvolutionFwdAlgo_t algorithm = CUDNN_CONVOLUTION_FWD_ALGO_FFT;
    // const cudnnConvolutionFwdAlgo_t algorithm = CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD;

    // Workspace
    size_t workspace_size;
    cudnnGetConvolutionForwardWorkspaceSize(nn, input_desc, filter_desc, convolution_desc, output_desc, algorithm, &workspace_size);
    ...
    for (size_t i = 0; i < 10; ++i)
        cudnnConvolutionForward(nn, &alpha, input_desc, input, filter_desc, filter, convolution_desc, algorithm, workspace, workspace_size, &beta, output_desc, output);
