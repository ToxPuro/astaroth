...
typedef AcReal (*boundcond_kernel_func)(AcReal boundary_val, AcReal domain_val, size_t r, AcMeshInfo info);
...
//Tests for "simple", generic boundconds, that apply a function of type boundcond_kernel_func to produce the ghost zone values
TestResultRegion
        ...
        for (int x = 0; x < dims.x; x++) {
            for (int y = 0; y < dims.y; y++) {
                for (int z = 0; z < dims.z; z++) {
                    ...
                    int3 boundary = int3{
                                        (direction.x == 0) ? dom.x : (direction.x==1) ? domain_start.x + dims.x :domain_start.x - 1 ,
                                        (direction.y == 0) ? dom.y : (direction.y==1) ? domain_start.y + dims.y :domain_start.y - 1 ,
                                        (direction.z == 0) ? dom.z : (direction.z==1) ? domain_start.z + dims.z :domain_start.z - 1};
                    ...
                    size_t r = std::abs(ghost.x - dom.x) + 
                               std::abs(ghost.y - dom.y) +
                               std::abs(ghost.z - dom.z);

                    AcReal expected_val = kernel_func(field[idx_bound], field[idx_dom], r/2, info);
                    if ((expected_val < field[idx_ghost] - epsilon) ||
                        (expected_val > field[idx_ghost] + epsilon)) {
                        res.field_errors.back().errors++;
                        res.failed_cells.push_back(CellError{(Field)i, dom, ghost, expected_val, field[idx_ghost]});
                    }
                }
            }
        }
...
//Tests for any boundconds by comparing the result to an entire mesh that constitutes the expected result
TestResultRegion
...
RunSimpleTest(SimpleTestCase test, AcMeshInfo info){
    ...
    TestResult result{test.name, true, true, true, true,std::array<TestResultRegion,6>(), std::array<TestResultRegion,12>(), std::array<TestResultRegion,8>()};
    ...
    if (pid == 0) {
        ...
        result.corner_regions[1] = test_simple_bc(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{nn.x - 1, nn.y - 1, NGHOST + 1},
                                      int3{nn.x + NGHOST, nn.y + NGHOST, 0}, info, test.test_func);
        result.corner_regions[2] = test_simple_bc(mesh, int3{0, 0, 1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{nn.x - 1, NGHOST + 1, nn.z - 1},
                                      int3{nn.x + NGHOST, 0, nn.z + NGHOST}, info, test.test_func);
        result.corner_regions[3] = test_simple_bc(mesh, int3{0, 0, 1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{NGHOST + 1, nn.y - 1, nn.z - 1},
                                      int3{0, nn.y + NGHOST, nn.z + NGHOST}, info, test.test_func);

        result.corner_regions[4] = test_simple_bc(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{NGHOST + 1, nn.y - 1, NGHOST + 1},
                                      int3{0, nn.y + NGHOST, 0}, info, test.test_func);
        result.corner_regions[5] = test_simple_bc(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
        ...
        result.corner_regions[7] = test_simple_bc(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
        ...
        result.all_regions_passed = result.all_faces_passed && result.all_edges_passed && result.all_edges_passed;
...
RunMeshCompareTest(MeshCompareTestCase test){
    ...
    if (pid == 0) {
        ...
        result.face_regions[0] = test_bc_against_mesh(mesh, int3{1, 0, 0}, int3{NGHOST, nn.y, nn.z},
                                    int3{nn.x - 1, NGHOST, NGHOST},
                                    int3{nn.x + NGHOST, NGHOST, NGHOST}, info, test.mesh_expected_result);

        result.face_regions[1] = test_bc_against_mesh(mesh, int3{0, 1, 0}, int3{nn.x, NGHOST, nn.z},
                                    int3{NGHOST, nn.y - 1, NGHOST},
                                    int3{NGHOST, nn.y + NGHOST, NGHOST}, info, test.mesh_expected_result);

        result.face_regions[2] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{nn.x, nn.y, NGHOST},
                                    int3{NGHOST, NGHOST, nn.z - 1},
                                    int3{NGHOST, NGHOST, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.face_regions[3] = test_bc_against_mesh(mesh, int3{-1, 0, 0}, int3{NGHOST, nn.y, nn.z},
                                    int3{NGHOST + 1, NGHOST, NGHOST}, int3{0, NGHOST, NGHOST},
                                    info, test.mesh_expected_result);

        result.face_regions[4] = test_bc_against_mesh(mesh, int3{0, -1, 0}, int3{nn.x, NGHOST, nn.z},
                                    int3{NGHOST, NGHOST + 1, NGHOST}, int3{NGHOST, 0, NGHOST},
                                    info, test.mesh_expected_result);

        result.face_regions[5] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{nn.x, nn.y, NGHOST},
                                    int3{NGHOST, NGHOST, NGHOST + 1}, int3{NGHOST, NGHOST, 0},
                                    info, test.mesh_expected_result);

        // edges
        result.edge_regions[0] = test_bc_against_mesh(mesh, int3{0, 1, 0}, int3{NGHOST, NGHOST, nn.z},
                                    int3{nn.x - 1, nn.y - 1, NGHOST},
                                    int3{nn.x + NGHOST, nn.y + NGHOST, NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[1] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{NGHOST, nn.y, NGHOST},
                                    int3{nn.x - 1, NGHOST, nn.z - 1},
                                    int3{nn.x + NGHOST, NGHOST, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[2] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{nn.x, NGHOST, NGHOST},
                                    int3{NGHOST, nn.y - 1, nn.z - 1},
                                    int3{NGHOST, nn.y + NGHOST, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[3] = test_bc_against_mesh(mesh, int3{0, -1, 0}, int3{NGHOST, NGHOST, nn.z},
                                    int3{nn.x - 1, NGHOST + 1, NGHOST},
                                    int3{nn.x + NGHOST, 0, NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[4] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{NGHOST, nn.y, NGHOST},
                                    int3{nn.x - 1, NGHOST, NGHOST + 1},
                                    int3{nn.x + NGHOST, NGHOST, 0}, info, test.mesh_expected_result);

        result.edge_regions[5] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{nn.x, NGHOST, NGHOST},
                                    int3{NGHOST, nn.y - 1, NGHOST + 1},
                                    int3{NGHOST, nn.y + NGHOST, 0}, info, test.mesh_expected_result);

        result.edge_regions[6] = test_bc_against_mesh(mesh, int3{0, 1, 0}, int3{NGHOST, NGHOST, nn.z},
                                    int3{NGHOST + 1, nn.y - 1, NGHOST},
                                    int3{0, nn.y + NGHOST, NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[7] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{NGHOST, nn.y, NGHOST},
                                    int3{NGHOST + 1, NGHOST, nn.z - 1},
                                    int3{0, NGHOST, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[8] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{nn.x, NGHOST, NGHOST},
                                    int3{NGHOST, NGHOST + 1, nn.z - 1},
                                    int3{NGHOST, 0, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.edge_regions[9] = test_bc_against_mesh(mesh, int3{0, -1, 0}, int3{NGHOST, NGHOST, nn.z},
                                    int3{NGHOST + 1, NGHOST + 1, NGHOST}, int3{0, 0, NGHOST},
                                    info, test.mesh_expected_result);

        result.edge_regions[10] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{NGHOST, nn.y, NGHOST},
                                    int3{NGHOST + 1, NGHOST, NGHOST + 1}, int3{0, NGHOST, 0},
                                    info, test.mesh_expected_result);

        result.edge_regions[11] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{nn.x, NGHOST, NGHOST},
                                    int3{NGHOST, NGHOST + 1, NGHOST + 1}, int3{NGHOST, 0, 0},
                                    info, test.mesh_expected_result);

        // result.corners
        result.corner_regions[0] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{nn.x - 1, nn.y - 1, nn.z - 1},
                                      int3{nn.x + NGHOST, nn.y + NGHOST, nn.z + NGHOST},
                                      info, test.mesh_expected_result);

        result.corner_regions[1] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{nn.x - 1, nn.y - 1, NGHOST + 1},
                                      int3{nn.x + NGHOST, nn.y + NGHOST, 0}, info, test.mesh_expected_result);
        result.corner_regions[2] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{nn.x - 1, NGHOST + 1, nn.z - 1},
                                      int3{nn.x + NGHOST, 0, nn.z + NGHOST}, info, test.mesh_expected_result);
        result.corner_regions[3] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{NGHOST + 1, nn.y - 1, nn.z - 1},
                                      int3{0, nn.y + NGHOST, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.corner_regions[4] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{NGHOST + 1, nn.y - 1, NGHOST + 1},
                                      int3{0, nn.y + NGHOST, 0}, info, test.mesh_expected_result);
        result.corner_regions[5] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{nn.x - 1, NGHOST + 1, NGHOST + 1},
                                      int3{nn.x + NGHOST, 0, 0}, info, test.mesh_expected_result);
        result.corner_regions[6] = test_bc_against_mesh(mesh, int3{0, 0, 1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{NGHOST + 1, NGHOST + 1, nn.z - 1},
                                      int3{0, 0, nn.z + NGHOST}, info, test.mesh_expected_result);

        result.corner_regions[7] = test_bc_against_mesh(mesh, int3{0, 0, -1}, int3{NGHOST, NGHOST, NGHOST},
                                      int3{NGHOST + 1, NGHOST + 1, NGHOST + 1}, int3{0, 0, 0},
                                      info, test.mesh_expected_result);
...
PrintTestResultRegion(const TestResultRegion &region)
{
    ...
    printf("\n\tRegion with normal (%2d,%2d,%d):\n\t", region.normal.x, region.normal.y, region.normal.z);

    for (int i = 0; i < NUM_VTXBUF_HANDLES; i++){
        size_t errors = region.field_errors[i].errors;
        size_t total = region.field_errors[i].total;

        printf("%*s: %s%lu%s/%s%lu%s.", col_width[i % 4], vtxbuf_names[i],
            (errors > 0 ? RED : GRN), (total - errors), RESET, GRN, total, RESET);
        if ((i + 1) % 4 == 0) {
            printf("\n\t");
        }   
    }
    ...
    for(size_t i = 0; i < region.failed_cells.size() && i < n_errors_to_print; i++){
        ...
        printf("\t\t%*s[(%3d,%3d,%3d)], domain index %*s[(%3d,%3d,%3d)]\n\t\texpected %s%f%s, got %s%f%s\n",
               col_width[error.field % 4], vtxbuf_names[error.field], error.ghost.x, error.ghost.y, error.ghost.z, 
               col_width[error.field % 4], vtxbuf_names[error.field], error.dom.x, error.dom.y, error.dom.z, 
               GRN, (double)error.produced, RESET,
               RED, (double)error.expected, RESET);
        fflush(stdout);
    }
#endif
...
main(void)
{
    ...
    test_cases.push_back(SimpleTestCase{"Prescribed derivative boundconds", prescribed_der_bc_graph, der_bc_func});
