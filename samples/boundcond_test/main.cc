        ...
        for (int x = 0; x < dims.x; x++) {
            for (int y = 0; y < dims.y; y++) {
                for (int z = 0; z < dims.z; z++) {
                    ...
                    //I think the boundary is calculated wrong when direction is -1
                    int3 boundary = int3{
...
TestResult
RunTest(TestCase test, AcMeshInfo info){
    int pid;
...
template<size_t n_regions>
void
colored_pass_ratio(const char* s, std::array<TestResultRegion,n_regions> &regions)
{
    size_t n_passed = 0;
    for (size_t i = 0; i < n_regions; i++){
        if (regions[i].passed){
...
void
PrintTestResultRegion(TestResultRegion &region)
{
...
void
PrintTestResult(TestResult result)
{
    printf("Test case ");
...
main(void)
{
    ...
    test_cases.push_back(TestCase{"Symmetric boundconds", symmetric_bc_graph, mirror});
    ...
    test_cases.push_back(TestCase{"Prescribed derivative boundconds", prescribed_der_bc_graph, der_bc_func});
    ...
    test_cases.push_back(TestCase{"Relative antisymmetry boundconds", relative_antisymmetry_bc_graph, a2_func});
    ...
    for (const auto &test: test_cases){
        test_results.push_back(RunTest(test, info));
