			...
			for(int i = 0; i < NUM_STENCILS; ++i)
			{
				...
				if(run_time && !comp_time)
					fatal("In Kernel %s Stencil %s used for %s at runtime but not generated!\n"
					      ...
					      "Consider either runtime-compilation or rewriting the kernel to avoid the conditional stencil call\n"
...
{ 
	...
	const Volume start = op.start;
	  ...
	  break;
	    ...
	    const bool raytracing = is_raytracing_kernel(op.analysis_info);
	...
	{
		const AcKernel kernel = std::dynamic_pointer_cast<ComputeTask>(dept_task)->getKernel();
		const KernelAnalysisInfo task_info = kernel_analysis_info[kernel];
		if(kernel_has_profile_stencil_ops(task_info))
		{
