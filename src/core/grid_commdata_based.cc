                   ...
                   CommData* data)
{
    ...
    const int3 pid3d        = getPid3D(pid, decomp, local_config.int_params[AC_proc_mapping_strategy]);
...
acGridInit(const AcMeshInfo info)
{
    ...
    const int3 pid3d             = getPid3D(pid, decomposition, info.int_params[AC_proc_mapping_strategy]);
        ...
        for (int k = 0; k < mm.z; ++k) {
            for (int j = 0; j < mm.y; ++j) {
                ...
                if (pid != 0) {
                    ...
                    for (int tgt_pid = 1; tgt_pid < nprocs; ++tgt_pid) {
                        const int3 tgt_pid3d = getPid3D(tgt_pid, grid.decomposition, grid.submesh.info[AC_proc_mapping_strategy]);
