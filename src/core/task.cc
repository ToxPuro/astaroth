...
HaloMessage::HaloMessage(int3 dims, MPI_Request* _req)
    :request(_req)
{
...
//HaloExchangeTask
HaloExchangeTask::HaloExchangeTask(const Device _device, const int _rank, const int3 _segment_id, const uint3_64 decomposition,
                                   const int3 grid_dimensions, MPI_Request* recv_requests, MPI_Request* send_requests)
...
HaloExchangeTask::receiveDevice()
{
    ...
    MPI_Irecv(msg->buffer.data, msg->length, AC_MPI_TYPE, recv_rank, recv_tag+HALO_TAG_OFFSET, MPI_COMM_WORLD, msg->request);
    msg->buffer.pinned = false;
}
...
HaloExchangeTask::receiveHost()
{
    if (onTheSameNode(rank,recv_rank)){
        ...
        auto msg = recv_buffers->get_fresh_buffer();
        MPI_Irecv(msg->buffer.data_pinned, msg->length, AC_MPI_TYPE, recv_rank, recv_tag+HALO_TAG_OFFSET, MPI_COMM_WORLD, msg->request);
...
HaloExchangeTask::sendDevice()
{
    auto msg = send_buffers->get_current_buffer();
    sync();
    MPI_Isend(msg->buffer.data, msg->length, AC_MPI_TYPE, send_rank, send_tag+HALO_TAG_OFFSET, MPI_COMM_WORLD, msg->request);
}
...
HaloExchangeTask::sendHost()
{
    ...
    if (onTheSameNode(rank,send_rank)){
        ...
        sync();
        MPI_Isend(msg->buffer.data_pinned, msg->length, AC_MPI_TYPE, send_rank, send_tag+HALO_TAG_OFFSET, MPI_COMM_WORLD, msg->request);
    }
}
