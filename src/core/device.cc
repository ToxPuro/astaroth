...
inline void
print_int3(const char* str, const int3 val)
{
...
// Corners
    ...
    for (size_t nstep = 0; nstep < nsteps; ++nstep) {
        for (int isubstep = 0; isubstep < 3; ++isubstep) {

            // Sync outer (needed for packing and inner integr.)
            acDeviceSynchronizeStream(device, STREAM_0);
            acDeviceSynchronizeStream(device, STREAM_1);
            acDeviceSynchronizeStream(device, STREAM_2);
            acDeviceSynchronizeStream(device, STREAM_3);
            acDeviceSynchronizeStream(device, STREAM_4);
            acDeviceSynchronizeStream(device, STREAM_5);
    /*
    #if MPI_COMPUTE_ENABLED
            /////////// INNER INTEGRATION //////////////
            const int3 nnmin = (int3){2 * NGHOST, 2 * NGHOST, 2 * NGHOST};
            const int3 nnmax = nn;
            { // Front
                const int3 m1 = (int3){nnmin.x, nnmin.y, nnmin.z};
                const int3 m2 = (int3){nnmax.x, nnmax.y, nnmin.z + NGHOST};
                acKernelIntegrateSubstep(device->streams[STREAM_6], isubstep, m1, m2, device->vba);
            }
            { // Back
                const int3 m1 = (int3){nnmin.x, nnmin.y, nnmax.z - NGHOST};
                const int3 m2 = (int3){nnmax.x, nnmax.y, nnmax.z};
                acKernelIntegrateSubstep(device->streams[STREAM_7], isubstep, m1, m2, device->vba);
            }
            { // Bottom
                const int3 m1 = (int3){nnmin.x, nnmin.y, nnmin.z + NGHOST};
                const int3 m2 = (int3){nnmax.x, nnmin.y + NGHOST, nnmax.z - NGHOST};
                acKernelIntegrateSubstep(device->streams[STREAM_8], isubstep, m1, m2, device->vba);
            }
            { // Top
                const int3 m1 = (int3){nnmin.x, nnmax.y - NGHOST, nnmin.z + NGHOST};
                const int3 m2 = (int3){nnmax.x, nnmax.y, nnmax.z - NGHOST};
                acKernelIntegrateSubstep(device->streams[STREAM_9], isubstep, m1, m2, device->vba);
            }
            { // Left
                const int3 m1 = (int3){nnmin.x, nnmin.y + NGHOST, nnmin.z + NGHOST};
                const int3 m2 = (int3){nnmin.x + NGHOST, nnmax.y - NGHOST, nnmax.z - NGHOST};
                acKernelIntegrateSubstep(device->streams[STREAM_10], isubstep, m1, m2, device->vba);
            }
            { // Right
                const int3 m1 = (int3){nnmax.x - NGHOST, nnmin.y + NGHOST, nnmin.z + NGHOST};
                const int3 m2 = (int3){nnmax.x, nnmax.y - NGHOST, nnmax.z - NGHOST};
                acKernelIntegrateSubstep(device->streams[STREAM_11], isubstep, m1, m2, device->vba);
            }
    #endif // MPI_COMPUTE_ENABLED
    */
    ...
    /*
    #if MPI_COMPUTE_ENABLED
            //////////// INNER INNER INTEGRATION //////////////
            {
                const int3 m1 = (int3){3 * NGHOST, 3 * NGHOST, 3 * NGHOST};
                const int3 m2 = nn - (int3){NGHOST, NGHOST, NGHOST};
                acKernelIntegrateSubstep(device->streams[STREAM_16], isubstep, m1, m2, device->vba);
            }
    ////////////////////////////////////////////
    #endif // MPI_COMPUTE_ENABLED
