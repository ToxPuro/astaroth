...
template <typename T>
static void
print(const std::string& label, const T& expr)
{
    std::cout << label << ": " << expr << std::endl;
}

static void
print(const std::string& label, const vec3& vec)
{
    std::cout << label << ": ";
...
template <typename T>
static void
print(const std::string& label, const std::complex<T>& c)
{
...
static auto
operator*(const double fac, const vec3& vec)
{
    return vec3{
        fac * vec[0],
        fac * vec[1],
...
static auto
operator*(const vec3& vec, const double fac)
{
    return vec3{
        vec[0] * fac,
        vec[1] * fac,
...
static auto
operator/(const vec3& vec, const double denom)
{
    return vec3{
        vec[0] / denom,
        vec[1] / denom,
...
static auto
angle(const vec3& a, const vec3& b)
{
    return std::acos(dot(a, b) / (norm(a) * norm(b)));
}

static auto
allclose(const double a, const double b, const double rtol = 1e-5, const double atol = 1e-8)
{
    return std::abs(a - b) <= (atol + rtol * std::abs(b));
}

static auto
allclose(const vec3& a, const vec3& b, const double rtol = 1e-5, const double atol = 1e-8)
{
 ...
 */
static double
gen_random_uniform_real(const double low, const double high)
{
 ...
 */
static int
gen_random_uniform_int(const int low, const int high)
{
    // static std::random_device rd;
    // static std::mt19937 gen{rd()};
 ...
 */
static vec3
gen_random_uniform_unit_vector()
{
    auto gen_vec = [] {
        return vec3{
 ...
 * interval [kmin, kmax] */
static auto
gen_k(const double kmin, const double kmax)
{
...
static auto
gen_f_k(const vec3& k, const vec3& e, const double relhel)
{
...
gen_phi()
{
    return gen_random_uniform_real(-M_PI, M_PI);
}
...
ForcingParams
generateHelicalForcingParams(const AcReal relhel, const AcReal magnitude, const AcReal kmin,
                             const AcReal kmax)
{
