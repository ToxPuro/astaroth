...
{
    uint8_t* arr = static_cast<uint8_t*>(malloc(bytes));
    errchk(arr);
    return arr;
}
...
{
    return static_cast<uint8_t*>(ac::mr::pinned_host_allocator::alloc(bytes));
}
...
#pragma omp parallel for
...

    {
        ...
        measurebw("Bidirectional bandwidth, async multiple (Host)", //
                  2 * (nprocs - 1) * BLOCK_SIZE,
                  sendrecv_nonblocking_multiple,
                  src,
                  dst);
...

    {
        ...
        measurebw("Bidirectional bandwidth, async multiple (Device, rt pinning)", //
                  2 * (nprocs - 1) * BLOCK_SIZE,
                  sendrecv_nonblocking_multiple_rt_pinning,
                  src,
                  dst);
...

    {
        ...
        measurebw2("Bidirectional D2H & H2D",
                   2 * BLOCK_SIZE,
                   sendrecv_d2h2d,
                   dsrc,
                   hdst,
...

    {
        ...
        measurebw2("Bidirectional D2H & H2D (pinned)",
                   2 * BLOCK_SIZE,
                   sendrecv_d2h2d,
                   dsrc,
                   hdst,
