...
template <typename T>
static int
verify_halo_exchange_mpi_hindexed(const MPI_Comm& cart_comm, const ac::shape& global_nn,
                                  const ac::index& rr)
{
    std::array<ac::host_ndbuffer<T>, 4> gbufs{
        ac::host_ndbuffer<T>{global_nn},
    ...
    ac::mpi::hindexed::halo_exchange<T, ac::mr::device_allocator> he{cart_comm,
                                                                     global_nn,
                                                                     rr,
                                                                     lptrs,
                                                                     lptrs};
...
verify(const MPI_Comm& cart_comm, const ac::shape& global_nn, const ac::index& rr)
{
    ...
    ERRCHK_MPI(verify_halo_exchange_mpi_hindexed<int>(cart_comm, global_nn, rr) == 0);
}
