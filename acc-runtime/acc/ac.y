...
split_user_kernels()
{
  ...
  for(int file = 0; file < NUM_FILES; ++file)
  {
    ...
    snprintf(filename,BUFFER_SIZE,"kernel_%d.cu",file % NUM_FILES);
    FILE *stream = fdopen(
      openat(
  ...
  while (kernel_start != NULL) {
    ...
    *kernel_name_end = '\0';
    ...
    char filename[BUFFER_SIZE] = {0};
    snprintf(filename,BUFFER_SIZE,"kernel_%d.cu",counter % NUM_FILES);
...
half_precision: HALF_PRECISION { $$ = astnode_create(NODE_UNKNOWN, NULL, NULL); astnode_set_buffer(yytext, $$); $$->token = 255 + yytoken; astnode_set_postfix(" ", $$); };
dead:      DEAD        { 
...
exp_sum: EXP_SUM { $$ = astnode_create(NODE_UNKNOWN, NULL, NULL); astnode_set_buffer("exp_sum", $$); $$->token = 255 + yytoken; };
...
type_qualifier: sum          { $$ = astnode_create(NODE_TQUAL, $1, NULL); }
              ...
              | half_precision { $$ = astnode_create(NODE_TQUAL, $1, NULL); }
...
struct_access_expression:
                   base_identifier '.' identifier          { $$ = astnode_create(NODE_STRUCT_EXPRESSION, $1, $3); astnode_set_infix(".", $$); set_identifier_type(NODE_MEMBER_ID, $$->rhs); }
		  | func_call '.' identifier               { $$ = astnode_create(NODE_STRUCT_EXPRESSION, $1, $3); astnode_set_infix(".", $$); set_identifier_type(NODE_MEMBER_ID, $$->rhs); }
...
function_call: declaration '(' ')'                 { $$ = astnode_create(NODE_FUNCTION_CALL, $1, NULL); astnode_set_infix("(", $$); astnode_set_postfix(")", $$); }
             ...
             | continue_node { $$ = astnode_create(NODE_UNKNOWN, $1, NULL); }
             ;
