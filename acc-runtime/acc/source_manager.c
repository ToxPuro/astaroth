...
acc_source_function_add_impl(AccSourceFunction* self, const char* format, ...)
{
  ...
  vasprintf(&added_impl, format, args);
  va_end(args);

  if (self->implementation != NULL) {
    char* new_implementation = NULL;
    asprintf(&new_implementation, "%s\n%s", self->implementation, added_impl);
                        ...
                        const char* format, ...)
{
  ...
  if (self->funcs_n >= self->funcs_capacity) {
    ...
    self->funcs = realloc(self->funcs,
                          self->funcs_capacity * sizeof(AccSourceFunction));
  }
...
{
  if (flags & ACC_SRC_DEVICE)
    return "cu";
  else if (flags & ACC_SRC_CPP)
    return "cpp";
  else
    return "c";
}
                               ...
                               AccSourceFlags flags)
{
  ...
  if (self->sources_n >= self->capacity) {
    self->capacity *= 2;
    self->sources = realloc(self->sources, self->capacity * sizeof(AccSource));
  }
