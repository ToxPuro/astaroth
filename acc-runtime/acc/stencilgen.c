...
gen_kernel_body(const int curr_kernel)
{
  ...
  case IMPLICIT_CACHING: {
    ...
    for (int depth_iter = 0; depth_iter < STENCIL_DEPTH; ++depth_iter) {
      for (int height_iter = 0; height_iter < STENCIL_HEIGHT; ++height_iter) {
        for (int width_iter = 0; width_iter < STENCIL_WIDTH; ++width_iter) {
          for (int stencil = 0; stencil < NUM_STENCILS; ++stencil) {
            for (int field = 0; field < NUM_FIELDS; ++field) {
              int depth = depth_order[depth_iter];
              int height = height_order[height_iter];
              int width = width_order[width_iter];
              // Skip if the stencil is not used
              if (!stencils_accessed[curr_kernel][field][stencil])
                continue;
