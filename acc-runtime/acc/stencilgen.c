...
void
gen_stencil_definitions(bool gen_analysis)
{
  if (!NUM_ALL_FIELDS)
  {
  ...
  printf(
      "%s__device__ __attribute__((unused)) /*const*/ AcReal /*__restrict__*/ "
...
gen_kernel_write_funcs(const int curr_kernel)
{
  ...
  // Write vba.out
	...
	for(int original_field = 0; original_field < NUM_ALL_FIELDS; ++original_field)
	{
		if(!write_called[curr_kernel][original_field]) continue;
  	      	const int field = get_original_index(field_remappings,original_field);
		if(vtxbuf_precision[field] != AC_REAL_PRECISION || vtxbuf_has_variable_dims[field] || is_x_raytrace_kernel(curr_kernel) || (accesses_z_ray(curr_kernel) && SHARED_MEM_Z_RAYS))
		{
...
main(int argc, char** argv)
{
  ...
  if (argc == 2 && !strcmp(argv[1], "-definitions-analysis")) {
    gen_stencil_definitions(true);
  }
  else if (argc == 2 && !strcmp(argv[1], "-definitions-kernels")) {
