...
acVBACreate(const AcMeshInfo config)
{
  ...
  const size_t in_bytes  = real_in_bytes + single_in_bytes + half_in_bytes;
  ...
  if((vba_in_buff == NULL && in_bytes != 0) || (vba_out_buff == NULL && out_bytes != 0))
  {
    ...
    fflush(stderr);
    ERRCHK_ALWAYS(vba_in_buff != NULL); //Was not able to allocate (most likely out of memory)
    ERRCHK_ALWAYS(vba_out_buff != NULL); //Was not able to allocate (most likely out of memory)
  }
