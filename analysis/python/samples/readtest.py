...
if (
    meshdir
    == "/tiara/ara/data/mvaisala/202202_acc3test/astaroth/config/samples/shockturb/"
):
    rlnrho = [-0.8, 0.6]
    rrho = [0.5, 2.0]
    rNcol = [100.0, 170.0]
    ...
    for i in range(0, 171):
        ...
        if mesh.ok:
            ...
            print(
                "lnrho",
                i,
                mesh.lnrho[(mesh.lnrho.shape[0] - 7) : mesh.lnrho.shape[0], 20, 100],
            )
            print(
                "uux",
                i,
                mesh.uu[0][(mesh.lnrho.shape[0] - 7) : mesh.lnrho.shape[0], 20, 100],
            )
            print(
                "uuy",
                i,
                mesh.uu[1][(mesh.lnrho.shape[0] - 7) : mesh.lnrho.shape[0], 20, 100],
            )
            print(
                "uuz",
                i,
                mesh.uu[2][(mesh.lnrho.shape[0] - 7) : mesh.lnrho.shape[0], 20, 100],
            )
            print(
                "uu_tot",
                i,
                uu_tot[(mesh.lnrho.shape[0] - 7) : mesh.lnrho.shape[0], 20, 100],
            )
...
if "diff" in sys.argv:
    ...
    vis.slices.plot_3(
        mesh1,
        mesh1.lnrho - mesh0.lnrho,
        title=r"$\ln \rho$",
        bitmap=True,
        fname="lnrho",
    )
    vis.slices.plot_3(
        mesh1, mesh1.uu[0] - mesh0.uu[0], title=r"$u_x$", bitmap=True, fname="uux"
    )
    vis.slices.plot_3(
        mesh1, mesh1.uu[1] - mesh0.uu[1], title=r"$u_y$", bitmap=True, fname="uuy"
    )
    vis.slices.plot_3(
        mesh1, mesh1.uu[2] - mesh0.uu[2], title=r"$u_z$", bitmap=True, fname="uuz"
    )
    ...
    for i in range(0, 100001, 1000):
        ...
        if mesh.ok:
            ...
            if "lnrho" in sys.argv:
                ...
                uu_tot = np.sqrt(
                    mesh.uu[0] ** 2.0 + mesh.uu[1] ** 2.0 + mesh.uu[2] ** 2.0
                )
...
if "sl" in sys.argv:
    ...
    for i in mesh_file_numbers:
        mesh = ad.read.Mesh(
            i, fdir=meshdir, xsplit=xsplit, ysplit=ysplit, zsplit=zsplit
        )
        ...
        if mesh.ok:
            ...
            if hasattr(mesh, "aa"):
                aa_tot = np.sqrt(
                    mesh.aa[0] ** 2.0 + mesh.aa[1] ** 2.0 + mesh.aa[2] ** 2.0
                )
                mesh.Bfield(trim=True)
                bb_tot = np.sqrt(
                    mesh.bb[0] ** 2.0 + mesh.bb[1] ** 2.0 + mesh.bb[2] ** 2.0
                )
            ...
            if "sym" in sys.argv:
                ...
                maxucomp = np.amax(
                    [
                        np.amax(np.abs(mesh.uu[0])),
                        np.amax(np.abs(mesh.uu[1])),
                        np.amax(np.abs(mesh.uu[2])),
                    ]
                )
                maxacomp = np.amax(
                    [
                        np.amax(np.abs(mesh.aa[0])),
                        np.amax(np.abs(mesh.aa[1])),
                        np.amax(np.abs(mesh.aa[2])),
                    ]
                )
                maxbcomp = np.amax(
                    [
                        np.amax(np.abs(mesh.bb[0])),
                        np.amax(np.abs(mesh.bb[1])),
                        np.amax(np.abs(mesh.bb[2])),
                    ]
                )
                ...
                if hasattr(mesh, "aa"):
                    ...
                    if hasattr(mesh, "lnrho"):
                        if mesh.lnrho is not None:
                            ...
                            vis.slices.plot_3(
                                mesh,
                                np.exp(mesh.lnrho) - np.exp(mesh2.lnrho),
                                title=r"$\rho$",
                                bitmap=True,
                                fname="diff_rho",
                            )
    ...
    for i in mesh_file_numbers:
        ...
        if mesh.ok:
            ...
            print(
                ...
                np.amin(mesh.lnrho),
                "SUM",
                np.sum((mesh.lnrho) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.lnrho) ** 2.0)),
            )
            print(
                "uu_tot      MAX",
                np.amax(uu_tot),
            ...
            print(
                ...
                np.amin(mesh.uu[0]),
                "SUM",
                np.sum((mesh.uu[0]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.uu[0]) ** 2.0)),
            )
            print(
                "mesh.uu[1]  MAX",
                np.amax(mesh.uu[1]),
                ...
                np.amin(mesh.uu[1]),
                "SUM",
                np.sum((mesh.uu[1]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.uu[1]) ** 2.0)),
            )
            print(
                "mesh.uu[2]  MAX",
                np.amax(mesh.uu[2]),
                ...
                np.amin(mesh.uu[2]),
                "SUM",
                np.sum((mesh.uu[2]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.uu[2]) ** 2.0)),
            )
            print(
                "aa_tot      MAX",
                np.amax(aa_tot),
            ...
            print(
                ...
                np.amin(mesh.aa[0]),
                "SUM",
                np.sum((mesh.aa[0]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.aa[0]) ** 2.0)),
            )
            print(
                "mesh.aa[1]  MAX",
                np.amax(mesh.aa[1]),
                ...
                np.amin(mesh.aa[1]),
                "SUM",
                np.sum((mesh.aa[1]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.aa[1]) ** 2.0)),
            )
            print(
                "mesh.aa[2]  MAX",
                np.amax(mesh.aa[2]),
                ...
                np.amin(mesh.aa[2]),
                "SUM",
                np.sum((mesh.aa[2]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.aa[2]) ** 2.0)),
            )
            print(
                "bb_tot      MAX",
                np.amax(bb_tot),
            ...
            print(
                ...
                np.amin(mesh.bb[0]),
                "SUM",
                np.sum((mesh.bb[0]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.bb[0]) ** 2.0)),
            )
            print(
                "mesh.bb[1]  MAX",
                np.amax(mesh.bb[1]),
                ...
                np.amin(mesh.bb[1]),
                "SUM",
                np.sum((mesh.bb[1]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.bb[1]) ** 2.0)),
            )
            print(
                "mesh.bb[2]  MAX",
                np.amax(mesh.bb[2]),
                ...
                np.amin(mesh.bb[2]),
                "SUM",
                np.sum((mesh.bb[2]) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.bb[2]) ** 2.0)),
            )
            print(
                "mesh.shock  MAX",
                np.amax(mesh.shock),
                ...
                np.amin(mesh.shock),
                "SUM",
                np.sum((mesh.shock) ** 2.0),
                "RMS",
                np.sqrt(np.mean((mesh.shock) ** 2.0)),
            )
...
if "vol" in sys.argv:
    ...
    for i in mesh_file_numbers:
        ...
        print(" %i / %i" % (i, maxfiles))
        if mesh.ok:
            vis.slices.volume_render(
                mesh, val1={"variable": "btot", "min": 0.5, "max": 2.0, "opacity": 0.05}
            )
            vis.slices.volume_render(
                mesh, val1={"variable": "utot", "min": 0.5, "max": 2.0, "opacity": 0.05}
            )
            vis.slices.volume_render(
                mesh,
                val1={"variable": "rho", "min": 10.0, "max": 300.0, "opacity": 0.05},
            )
            vis.slices.volume_render(
                mesh, val1={"variable": "aa", "min": 0.1, "max": 0.25, "opacity": 0.05}
            )
...
if "tscomp" in sys.argv:
    ...
    plt.plot(
        ts_orig.var["t_step"][:-1],
        ts_orig.var["uutot_rms"][:-1],
        label="uu_total_rms (orig)",
    )
    # plt.xlabel(xaxis)
    plt.legend()
    ...
    plt.plot(
        ts_orig.var["t_step"][:-1],
        ts_orig.var["bbtot_rms"][:-1],
        label="bb_total_rms (orig)",
    )
    # plt.xlabel(xaxis)
    plt.legend()
    ...
    plt.plot(
        ts_orig.var["t_step"][:-1],
        ts_orig.var["shock_rms"][:-1],
        label="VTXBUF_SHOCK_rms (orig)",
    )
    # plt.xlabel(xaxis)
    plt.legend()
    ...
    plt.plot(
        ts_orig.var["t_step"][:-1],
        ts_orig.var["shock_max"][:-1],
        label="VTXBUF_SHOCK_max (orig)",
    )
    # plt.xlabel(xaxis)
    plt.legend()
    ...
    plt.plot(
        ts_orig.var["t_step"][:-1],
        ts_orig.var["lnrho_max"][:-1],
        label="lnrho max (orig)",
    )
    # plt.xlabel(xaxis)
    plt.legend()
...
if ("3drend" in sys.argv) and pv_present:
    ...
    for i in mesh_file_numbers:
        ...
        if mesh.ok:
            ...
            mesh.uu = (
                mesh.uu[0][3:-3, 3:-3, 3:-3],
                mesh.uu[1][3:-3, 3:-3, 3:-3],
                mesh.uu[2][3:-3, 3:-3, 3:-3],
            )
            mesh.aa = (
                mesh.aa[0][3:-3, 3:-3, 3:-3],
                mesh.aa[1][3:-3, 3:-3, 3:-3],
                mesh.aa[2][3:-3, 3:-3, 3:-3],
            )
            mesh.bb = (
                mesh.bb[0][3:-3, 3:-3, 3:-3] * um,
                mesh.bb[1][3:-3, 3:-3, 3:-3] * um,
                mesh.bb[2][3:-3, 3:-3, 3:-3] * um,
            )
            mesh.jj = (
                mesh.jj[0][3:-3, 3:-3, 3:-3],
                mesh.jj[1][3:-3, 3:-3, 3:-3],
                mesh.jj[2][3:-3, 3:-3, 3:-3],
            )
            ...
            grid.cell_arrays["Btot"] = np.sqrt(
                mesh.bb[0] ** 2.0 + mesh.bb[1] ** 2.0
            ).flatten(
                order="F"
            )  # Flatten the array!
            ...
            clim = [1e-6, 1000e-6]
            pp.add_volume(
                grid, mapper="gpu", cmap="plasma_r", clim=[1e-6, 1000e-6]
            )  # Pseudodisk Bxy, B0 = 3000 muG
    ...
    for i in mesh_file_numbers:
        ...
        if mesh.ok:
            if hasattr(mesh, "lnrho"):
                if mesh.lnrho is not None:
                    np.save(
                        "density_%s.npy" % (mesh.framenum), np.exp(mesh.lnrho[3:-3])
                    )
            if hasattr(mesh, "uu"):
                if mesh.uu[0] is not None:
