"""
Copyright (C) 2014-2023, Johannes Pekkilae, Miikka Vaeisalae.
...
def testmesh(resolution, dx, dy, dz):
    ...
    vecz = (
        np.sin(6.0 * zz) + np.sin(8.0 * zz) + np.cos(20.0 * xx) + 10 * np.cos(9.0 * yy)
    )
...
def extract_spectra(meshdirs, uu=False, noheltext=""):
    for meshdir in meshdirs:
        ...
        if len(mesh_file_numbers) > 0:
            ...
            headerinfo = (
                "nx %e, dsx %e, dsy %e, dsz %e, relhel %e, kaver %e, eta %e, nu %e \n"
            ...
            for i in pbar:
                ...
                if mesh.ok:
                    ...
                    Etot, Exx, freqs_x, Eyy, freqs_y, Ezz, freqs_z = powerspectra_shell(
                        mesh, dsx, dsy, dsz, uu=uu
                    )
                ...
                if writefreq:
                    etotfile.write(
                        "kk"
                        + " "
                        + np.array2string(freqs_x, max_line_width=1000000)
                        + "\n"
                    )
                    exfile.write(
                        "kx"
                    ...
                    eyfile.write(
                        "ky"
                        + " "
                        + np.array2string(freqs_y, max_line_width=1000000)
                        + "\n"
                    )
                    ezfile.write(
                        "kz"
                        + " "
                        + np.array2string(freqs_z, max_line_width=1000000)
                        + "\n"
                    )
                    writefreq = False
                ...
                etotfile.write(
                    str(mesh.timestamp)
                    + " "
                    + np.array2string(Etot, max_line_width=1000000)
                    + "\n"
                )
                exfile.write(
                    str(mesh.timestamp)
                    + " "
                    + np.array2string(Exx, max_line_width=1000000)
                    + "\n"
                )
                eyfile.write(
                    str(mesh.timestamp)
                    + " "
                    + np.array2string(Eyy, max_line_width=1000000)
                    + "\n"
                )
                ezfile.write(
                    str(mesh.timestamp)
                    + " "
                    + np.array2string(Ezz, max_line_width=1000000)
                    + "\n"
                )
...
if "xyaver" in sys.argv:
    ...
    for meshdir in meshdirs:
        ...
        datamin = np.amax(
            [Bx_xyt, By_xyt, Bz_xyt, Bx_xzt, By_xzt, Bz_xzt, Bx_yzt, By_yzt, Bz_yzt]
        )
        datamax = np.amin(
            [Bx_xyt, By_xyt, Bz_xyt, Bx_xzt, By_xzt, Bz_xzt, Bx_yzt, By_yzt, Bz_yzt]
        )
        ...
        colornorm = mpl.colors.SymLogNorm(
            linthresh=1e-6, vmin=-datarange, vmax=datarange
        )

        image1 = axs[0].pcolormesh(
            ttz,
            zz,
            np.transpose(Bx_xyt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image2 = axs[1].pcolormesh(
            ttz,
            zz,
            np.transpose(By_xyt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image3 = axs[2].pcolormesh(
            ttz,
            zz,
            np.transpose(Bz_xyt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image4 = axs[3].pcolormesh(
            tty,
            yy,
            np.transpose(Bx_xzt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image5 = axs[4].pcolormesh(
            tty,
            yy,
            np.transpose(By_xzt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image6 = axs[5].pcolormesh(
            tty,
            yy,
            np.transpose(Bz_xzt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image7 = axs[6].pcolormesh(
            ttx,
            xx,
            np.transpose(Bx_yzt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image8 = axs[7].pcolormesh(
            ttx,
            xx,
            np.transpose(By_yzt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
        image9 = axs[8].pcolormesh(
            ttx,
            xx,
            np.transpose(Bz_yzt[1:]),
            norm=colornorm,
            cmap=plt.get_cmap("inferno"),
        )
