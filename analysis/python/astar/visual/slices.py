...
def plot_3(
    ...
    yz_slice = yz_slice[
        trimghost : yz_slice.shape[0] - trimghost,
        trimghost : yz_slice.shape[1] - trimghost,
    ]
    xz_slice = xz_slice[
        trimghost : xz_slice.shape[0] - trimghost,
        trimghost : xz_slice.shape[1] - trimghost,
    ]
    xy_slice = xy_slice[
        trimghost : xy_slice.shape[0] - trimghost,
        trimghost : xy_slice.shape[1] - trimghost,
    ]
    mesh_xx_tmp = mesh.xx[
        (trimghost + trimaxis) : mesh.xx.shape[0] - (trimghost + trimaxis)
    ]
    mesh_yy_tmp = mesh.yy[
        (trimghost + trimaxis) : mesh.yy.shape[0] - (trimghost + trimaxis)
    ]
    mesh_zz_tmp = mesh.zz[
        (trimghost + trimaxis) : mesh.zz.shape[0] - (trimghost + trimaxis)
    ]
    ...
    if points_from_centre > 0:
        yz_slice = yz_slice[
            int(yz_slice.shape[0] / 2)
            - points_from_centre : int(yz_slice.shape[0] / 2)
            + points_from_centre,
            int(yz_slice.shape[1] / 2)
            - points_from_centre : int(yz_slice.shape[1] / 2)
            + points_from_centre,
        ]
        xz_slice = xz_slice[
            int(xz_slice.shape[0] / 2)
            - points_from_centre : int(xz_slice.shape[0] / 2)
            + points_from_centre,
            int(xz_slice.shape[1] / 2)
            - points_from_centre : int(xz_slice.shape[1] / 2)
            + points_from_centre,
        ]
        xy_slice = xy_slice[
            int(xy_slice.shape[0] / 2)
            - points_from_centre : int(xy_slice.shape[0] / 2)
            + points_from_centre,
            int(xy_slice.shape[1] / 2)
            - points_from_centre : int(xy_slice.shape[1] / 2)
            + points_from_centre,
        ]
        mesh_xx_tmp = mesh.xx[
            int(mesh.xx.shape[0] / 2)
            - points_from_centre : int(mesh.xx.shape[0] / 2)
            + points_from_centre
        ]
        mesh_yy_tmp = mesh.yy[
            int(mesh.yy.shape[0] / 2)
            - points_from_centre : int(mesh.yy.shape[0] / 2)
            + points_from_centre
        ]
        mesh_zz_tmp = mesh.zz[
            int(mesh.zz.shape[0] / 2)
            - points_from_centre : int(mesh.zz.shape[0] / 2)
            + points_from_centre
        ]
    ...
    if bfieldlines:
        ax00.streamplot(
            mesh.yy, mesh.zz, np.mean(mesh.bb[1], axis=0), np.mean(mesh.bb[2], axis=0)
        )
        ax10.streamplot(
            mesh.xx, mesh.zz, np.mean(mesh.bb[0], axis=1), np.mean(mesh.bb[2], axis=1)
        )
        ax11.streamplot(
            mesh.xx, mesh.yy, np.mean(mesh.bb[0], axis=2), np.mean(mesh.bb[1], axis=2)
        )
    ...
    if velfieldlines:
        ax00.streamplot(
            mesh.yy, mesh.zz, mesh.uu[2][mesh.xmid, :, :], mesh.uu[1][mesh.xmid, :, :]
        )
        ax10.streamplot(
            mesh.xx, mesh.zz, mesh.uu[2][:, mesh.ymid, :], mesh.uu[0][:, mesh.ymid, :]
        )
        ax11.streamplot(
...
def volume_render(
    mesh, val1={"variable": None, "min": None, "max": None, "opacity": 1.0}
):
