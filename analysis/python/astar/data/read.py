...
def read_bin(
    ...
    my_dtype = set_dtype(
        minfo.contents["endian"], minfo.contents["AcRealSize"], print_type=getfilename
    )
    ...
    if read_ok:
        ...
        if slices:
            array = np.reshape(
                array[1:], (minfo.contents["AC_mx"], minfo.contents["AC_my"]), order="F"
            )
        else:
            array = np.reshape(
                array[1:],
                (
                    minfo.contents["AC_mx"],
        ...
        array = np.zeros(
            (minfo.contents["AC_nx"], minfo.contents["AC_ny"], minfo.contents["AC_nz"])
        )
        ...
        for xind in xsplit:
            for yind in ysplit:
                for zind in zsplit:
                    ...
                    array_part = np.reshape(
                        array_part, (xpoints, ypoints, zpoints), order="F"
                    )
                    array[
                        xind : (xind + xpoints),
                        yind : (yind + ypoints),
...
def div(array, minfo):
    ...
    dy = minfo.contents["AC_dsy"]
    dz = minfo.contents["AC_dsz"]
    return DERX(array[0], dx) + DERY(array[1], dy) + DERZ(array[2], dz)
...
def grad(array, minfo):
    ...
    dy = minfo.contents["AC_dsy"]
    dz = minfo.contents["AC_dsz"]
    return (DERX(array, dx), DERY(array, dy), DERZ(array, dz))
...
def curl_of_curl(array, minfo):
    ...
    array2 = (
        apply_boundcond(array1[0], "p"),
        apply_boundcond(array1[1], "p"),
        apply_boundcond(array1[2], "p"),
    )
...
class Mesh:
    ...
    def __init__(
        ...
        if self.ok:
            ...
            self.shock, timestamp, ok = read_bin(
                "VTXBUF_SHOCK",
                fdir,
                fnum,
                xsplit,
            ...
            self.accretion, timestamp, ok = read_bin(
                "VTXBUF_ACCRETION",
                fdir,
                fnum,
                xsplit,
            ...
            self.xx = (
                np.arange(self.minfo.contents["AC_mx"]) * self.minfo.contents["AC_dsx"]
            )
            self.yy = (
                np.arange(self.minfo.contents["AC_my"]) * self.minfo.contents["AC_dsy"]
            )
            self.zz = (
                np.arange(self.minfo.contents["AC_mz"]) * self.minfo.contents["AC_dsz"]
            )
    ...
    def Bfield(self, get_jj=False, trim=False):
        ...
        if trim:
            self.bb = (
                self.bb[0][3:-3, 3:-3, 3:-3],
                self.bb[1][3:-3, 3:-3, 3:-3],
                self.bb[2][3:-3, 3:-3, 3:-3],
            )
            ...
            if get_jj:
                self.jj = (
                    ...
                    self.jj[1][3:-3, 3:-3, 3:-3],
                    self.jj[2][3:-3, 3:-3, 3:-3],
                )
    ...
    def Bfieldlines(self, footloc="default", vartype="B", maxstep=1000):
        ...
        if vartype == "U":
            # Trim to match
            self.uu = (
                self.uu[0][3:-3, 3:-3, 3:-3],
                self.uu[1][3:-3, 3:-3, 3:-3],
                self.uu[2][3:-3, 3:-3, 3:-3],
            )
        ...
        def field_line_step(self, coord, ds):
            ...
            Bcell_abs = np.sqrt(
                Bcell_vec[0] ** 2.0 + Bcell_vec[1] ** 2.0 + Bcell_vec[2] ** 2.0
            )
        ...
        if footloc == "middlez":
            ...
            zfoots = np.array(
                [(self.zz_trim.max() - self.zz_trim.min()) / 2.0 + self.zz_trim.min()]
            )
        elif footloc == "cube":
            ixtot = 5
            iytot = 5
            iztot = 5
            xfoots = np.linspace(
                self.xx_trim.min() + 3.0 * dx, self.xx_trim.max() - 3.0 * dx, num=ixtot
            )
            yfoots = np.linspace(
                self.yy_trim.min() + 3.0 * dy, self.yy_trim.max() - 3.0 * dy, num=iytot
            )
            zfoots = np.linspace(
                self.zz_trim.min() + 3.0 * dz, self.zz_trim.max() - 3.0 * dz, num=iztot
            )
        else:
            ixtot = 6
            iytot = 6
        ...
        for zfoot in zfoots:
            for yfoot in yfoots:
                for xfoot in xfoots:
                    ...
                    self.df_lines = self.df_lines.append(
                        {
                            "line_num": ii,
                            "dstot": dstot,
                            "coordx": coord[0],
                    ...
                    while integrate:
                        ...
                        if counter >= maxstep:
                            integrate = 0
                        if (
                            (coord[0] > self.xx_trim.max())
                            or (coord[1] > self.yy_trim.max())
                            or (coord[2] > self.zz_trim.max())
                            or (coord[0] < self.xx_trim.min())
                            or (coord[1] < self.yy_trim.min())
                            or (coord[2] < self.zz_trim.min())
                        ):
                            # print("out of bounds")
                            integrate = 0
                        if (
    ...
    def vorticity(self, trim=False):
        ...
        if trim:
            self.oo = (
                self.oo[0][3:-3, 3:-3, 3:-3],
                self.oo[1][3:-3, 3:-3, 3:-3],
                self.oo[2][3:-3, 3:-3, 3:-3],
            )
    ...
    def rad_vel(self):
        ...
        xx, yy, zz = np.meshgrid(
            self.xx - self.xmid, self.yy - self.ymid, self.zz - self.zmid
        )
    ...
    def yt_conversion(self):
        if yt_present:
            self.ytdict = dict(
                density=(
                    np.exp(self.lnrho) * self.minfo.contents["AC_unit_density"],
                    "g/cm**3",
                ),
            ...
            bbox = self.minfo.contents["AC_unit_length"] * np.array(
                [
                    [self.xx.min(), self.xx.max()],
                    [self.yy.min(), self.yy.max()],
                    [self.zz.min(), self.zz.max()],
                ]
            )
            self.ytdata = yt.load_uniform_grid(
                self.ytdict, self.lnrho.shape, length_unit="cm", bbox=bbox
            )
        else:
            ...
            for jj in np.arange(3, self.yy.size - 3):
                for ii in np.arange(3, self.xx.size - 3):
                    ...
                    linestring = "%e, %e, %e, %e, %e, %e, %e, %e, %e, %e\n" % (
                        ...
                        self.uu[0][ii, jj, kk] * uv,
                        self.uu[1][ii, jj, kk] * uv,
                        self.uu[2][ii, jj, kk] * uv,
                        self.bb[0][ii, jj, kk] * um,
                        self.bb[1][ii, jj, kk] * um,
    ...
    def export_raw(self):
        ...
        print(
            self.lnrho.shape,
            set_dtype(self.minfo.contents["endian"], self.minfo.contents["AcRealSize"]),
        )
    ...
    def export_vtk_ascii(self, Beq=1.0):
        ...
        Ntot = (
            self.minfo.contents["AC_mx"]
            * self.minfo.contents["AC_my"]
            * self.minfo.contents["AC_mz"]
        )
