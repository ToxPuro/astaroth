...
CMAKE_FLAGS="-DOPTIMIZE_MEM_ACCESSES=ON -DDOUBLE_PRECISION=ON -DMPI_ENABLED=OFF -DUSE_CUDA_AWARE_MPI=OFF"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --fedora44)
            ...
            CMAKE_FLAGS="$CMAKE_FLAGS -DBUILD_SAMPLES=OFF -DUSE_HIP=OFF -DUSE_CUDA_AWARE_MPI=OFF -DDSL_MODULE_DIR=../../../acc-runtime/samples/mhd_modular/ -DCMAKE_CXX_COMPILER=$HOME/gcc15/bin/g++ -DCMAKE_C_COMPILER=$HOME/gcc15/bin/gcc -DCMAKE_CUDA_HOST_COMPILER=$HOME/gcc15/bin/g++"
            CMAKE_FLAGS="$CMAKE_FLAGS -DMPI_C_INCLUDE_PATH=$HOME/openmpi/v5/include/ -DMPI_C_LIBRARIES=$HOME/openmpi/v5/lib/libmpi.so -DGPU_ARCH=100"
            ;;
        *)
