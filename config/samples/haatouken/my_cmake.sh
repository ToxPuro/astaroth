...
case $HOSTNAME in 
  ( gp8.tiara.sinica.edu.tw | gp9.tiara.sinica.edu.tw | gp10.tiara.sinica.edu.tw | gp11.tiara.sinica.edu.tw ) 
     cmake -DOPTIMIZE_MEM_ACCESSES=ON -DDOUBLE_PRECISION=ON -DUSE_HIP=OFF -DMPI_ENABLED=ON -DUSE_CUDA_AWARE_MPI=OFF -DDSL_MODULE_DIR=../../../acc-runtime/samples/mhd_modular/ -DCMAKE_CXX_COMPILER=/software/opt/gcc/9.1.0/bin/gcc -DCMAKE_C_COMPILER=/software/opt/gcc/9.1.0/bin/gcc ../../.. 
     ;;
  (*) 
     cmake -DOPTIMIZE_MEM_ACCESSES=ON -DDOUBLE_PRECISION=ON  -DUSE_HIP=OFF -DMPI_ENABLED=ON -DUSE_CUDA_AWARE_MPI=ON  ../../..
     ;;
esac
